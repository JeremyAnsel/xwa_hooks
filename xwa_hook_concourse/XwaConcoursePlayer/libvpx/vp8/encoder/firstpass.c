...
void vp8_first_pass(VP8_COMP *cpi) {
  ...
  if (0) {
    char filename[512];
    FILE *recon_file;
    sprintf_s(filename, sizeof(filename), "enc%04d.yuv", (int)cm->current_video_frame);

    if (cm->current_video_frame == 0) {
      fopen_s(&recon_file, filename, "wb");
    } else {
      fopen_s(&recon_file, filename, "ab");
    }
