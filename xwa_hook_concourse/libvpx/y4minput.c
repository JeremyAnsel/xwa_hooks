...
static int file_read(void *buf, size_t size, FILE *file) {
    ...
    if (file_error) {
      if (errno == EINTR || errno == EAGAIN) {
        ...
        fprintf(stderr, "Error reading file: %u of %u bytes read, %d: %s\n",
                (uint32_t)len, (uint32_t)size, errno, error);
        return 0;
      }
    }
  }
  ...
  if (!feof(file) && len != size) {
    ...
    fprintf(stderr,
            ...
            " error: %d, tries: %d, %d: %s\n",
            (uint32_t)len, (uint32_t)size, file_error, try_count, errno, error);
  }
  return len == size;
}
...
static int y4m_parse_tags(y4m_input *_y4m, char *_tags) {
  ...
  for (p = _tags;; p = q) {
    ...
    switch (p[0]) {
      case 'W': {
        if (sscanf_s(p + 1, "%d", &_y4m->pic_w) != 1) return -1;
        break;
      }
      case 'H': {
        if (sscanf_s(p + 1, "%d", &_y4m->pic_h) != 1) return -1;
        break;
      }
      case 'F': {
        if (sscanf_s(p + 1, "%d:%d", &_y4m->fps_n, &_y4m->fps_d) != 2) {
          return -1;
        }
        break;
      }
      ...
      case 'A': {
        if (sscanf_s(p + 1, "%d:%d", &_y4m->par_n, &_y4m->par_d) != 2) {
          return -1;
        }
        break;
      }
